...
>
> 这个项目可能算不上管理系统，项目初期是决定实现对数据的增删改，但到后面发现，游戏数据基本是写死的，只有增加，修改和删除基本用不上(也不太关心官方更新，英雄的技能微调)，最后偷懒只保留了增加。
>
> 在增加页面，由于考虑不周全，当时已经写好增加单个语音页面，发现语音数据量过于庞大，最后通过获取官方的语音`json`数据，所以就移除了语音增加页面。
>
> 当时也写好了增加装备页面，本来只是为了展示装备详情，但后面自己想增加点击装备自动连线的功能，这就需要在添加装备时设置合成装备，但合成的数据处理比较麻烦，添加装备的合成表需要某个装备，出现了比较尴尬的情况，需要的装备还没添加，再加上装备在游戏内很长时间都不会更新，最后移除了增加装备页面，本来打算留个纪念，但最后考虑到项目整洁，选择丢弃。
