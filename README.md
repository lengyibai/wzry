...
> 此项目用于个人积累项目经验，与同行一起交流与学习，顺便展示自己的能力，并无实际用途
>
...
> 
>
> 过程：项目经历过从`Vue2 + Cli`升级为`Vue3 + TS + Vite`，删除了`Vue2`时期的`Git历史`，也在当时，将项目的所有图片采用图床展示，使项目大小从原来的超过`40MB`降至不足`4MB`，减轻下载项目的压力。这个项目可能算不上管理系统，项目初期是决定实现对数据的增删改，但到后面发现，游戏数据基本是写死的，只有增加，修改和删除基本用不上(也不太关心官方更新，英雄的技能微调)，最后偷懒只保留了增加。并且在增加页面，由于考虑不周全，当时已经写好增加单个语音页面，发现语音数据量过于庞大，最后通过获取官方的语音`json`数据，所以就移除了语音增加页面。当时也写好了增加装备页面，本来只是为了展示装备详情，但后面自己想增加点击装备自动连线的功能，这就需要在添加装备时设置合成装备，但合成的数据处理比较麻烦，添加装备的合成表需要某个装备，出现了比较尴尬的情况，需要的装备还没添加，再加上装备在游戏内很长时间都不会更新，最后移除了增加装备页面，本来打算留个纪念，但最后考虑到项目整洁，选择丢弃。
...
> 5、悬浮皮肤卡片查看大图和语音
>
> 6、查看大图支持鼠标滚轮/点击按钮放大、拖动
>
> 7、查看语音支持暂停播放、自动播放下一个直至最后一个播放完后终止
