...
import { AudioStore, LotteryStore, MarkerStore } from "@/store";
...
const { setLuckValue, resetSkinLuck } = $lotteryStore;
  ...
  const play = () => {
    setTimeout(async () => {
      ...
      if (gift_index.value === active_index.value && awards_index.length < 5) {
        //增加幸运值的时候顺便扣除了夺宝币，只有抵扣石为0的时候会扣除
        setLuckValue("SKIN");
        ...
        await _promiseTimeout(getLotteryIndex, 500);

        //当奖品达到5个时，停止夺宝并结算奖励
        if (awards_index.length >= 5) {
          calcMultipleReward(awards_index);
          $markerStore.addMarkerNum("i71y", 5);
          return;
        }
      }
      play();
