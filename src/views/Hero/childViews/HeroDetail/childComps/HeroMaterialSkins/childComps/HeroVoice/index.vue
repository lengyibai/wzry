<template>
  <div class="HeroVoice">
    <div class="voice flex cursor-pointer" :class="{ active: currentIndex === index }" :style="{ 'animation-duration': time + 's' }" @click="play(item.voice, index)" v-for="(item, index) in voices" ref="voiceRef" :key="index">
      <span v-if="currentIndex !== index"> {{ item.desc }}</span>
      <transition name="fade">
        <LibSvg v-if="currentIndex === index" :svg="icon" color="#fff" enter-color="var(--blue)" down-color="var(--red)" size="35px" />
      </transition>
    </div>
    <!--播放语音-->
...
import { Voice } from '@/interface/hero'
...
let ended: () => void = () => {
  ...
  /* 等待播放动画结束后再播放 */
  setTimeout(() => {
    play(voices.value[currentIndex.value + 1].voice, currentIndex.value + 1);
  }, 250);
};
</script>
