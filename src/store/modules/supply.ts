...
import { AudioStore, AuthStore } from "..";

import { $confirmText, GAME_CONFIG, MESSAGE_TIP } from "@/config";
...
const SupplyStore = defineStore("supply", () => {
  ...
  const ExposeMethods = {
    ...
    useUserSupply(v: Global.UserData["supply"]) {
    ...
    startCountdown(type: "HERO" | "SKIN") {
      ...
      if (supply_remain_num <= 0) {
        $message(MESSAGE_TIP.xv24, "error");
        return;
      }
