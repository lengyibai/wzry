...
/** @description 皮肤碎片商城 */
const SkinDebrisStore = defineStore("skinDebris", () => {
  ...
  const sortAll = () => {
    ...
    const sortPrice = () => {
      ...
      const sort_strategy: Record<string, (list: Game.Hero.Skin[]) => Game.Hero.Skin[]> = {
        由低到高: (list) => {
          return list.sort((a, b) => Number(a.debris) - Number(b.debris));
        },
        由高到低: (list) => {
          return list.sort((a, b) => Number(b.debris) - Number(a.debris));
        },
      };
  ...
  const ExposeMethods = {
    ...
    async getSkin() {
      //筛除伴生、唯一限定、源梦、价值小于<6个皮肤碎片
      const skin_list = (await GAME_HERO.getSkinList()).filter(
        (item) =>
          item.debris &&
          Number(item.debris) > 6 &&
          ![0, 5, 65].includes(item.type) &&
