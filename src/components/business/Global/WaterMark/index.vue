...
/* 是否为旧版 */
const old = computed(
  () => $versionStore.local_version !== $versionStore.remote_version,
);
const old_file = computed(
  () => $versionStore.local_file !== $versionStore.file_version,
);
...
<template>
  ...
  >
    <p>{{ $t("帧率") }}：{{ fps }}</p>
    <p>
      {{ $t("屏幕尺寸") }}：{{ $deviceStore.width }}*{{ $deviceStore.height }}
    </p>
    <p :class="{ low: !browser_status }">
      {{ browser_name }}{{ $t("内核") }}：{{ version }}
    </p>
    <p :class="{ old: old }">
      {{ $t("数据") }}：{{ $versionStore.local_version }}
    </p>
    <p v-if="old" :class="{ new: old }">
      {{ $t("正在更新数据") }}：{{ $versionStore.remote_version }}
    </p>
    <p :class="{ old: old_file }">
      {{ $t("网页") }}：{{ $versionStore.local_file }}
    </p>
