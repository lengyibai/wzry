import { getRelationship } from "@/api/modules/games/relationship";
import { get } from "@/api/helper/transfer";
import { CONFIG } from "@/config";

/** @description 获取指定英雄关系 */
export const getHeroRelationship = async (hero_id: number) => {
  const relationship_list = await getRelationship();
  const relationship = relationship_list.find((item) => item.id === hero_id)!;
